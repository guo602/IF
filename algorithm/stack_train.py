...
def main(exp: mltk.Experiment[ExpConfig], config: ExpConfig):
    ...
    random.seed(np.random.randint(2**32, dtype=np.int64))
    tf.set_random_seed(np.random.randint(2**32, dtype=np.int64))
    np.random.seed(np.random.randint(2**32, dtype=np.int64))
